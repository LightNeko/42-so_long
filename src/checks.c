/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   checks.c                                           :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: znicola <znicola@student.42.fr>            +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/02/06 09:01:52 by znicola           #+#    #+#             */
/*   Updated: 2025/02/06 17:45:57 by znicola          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "so_long.h"

void	check_file_format(char *filename)
{
	char	*p_format;

	p_format = ft_strrchr(filename, '.');
	if (ft_strcmp(p_format, ".ber"))
		manage_error(1, "Invalid File Extension: file should be .ber\n");
}

static int	check_boundaries(t_map map)
{
	size_t	i;

	i = 0;
	while (i < map.llen)
	{
		if (map.map[0][i] != '1' || map.map[map.len - 1][i] != '1')
			return (1);
		i++;
	}
	i = 0;
	while (i < map.len)
	{
		if (map.map[i][0] != '1' || map.map[i][map.llen - 1] != '1')
			return (1);
		i++;
	}
	return (0);
}

void	check_line_content(t_data *data, char *l)
{
	char	*p;
	
	if (l == NULL)
		return;
	p = l;
	while (*p != '\n')
	{
		if (*p == '0' || *p == '1')
			;
		else if (*p == 'P')
			data->p.count++;
		else if (*p == 'C')
			data->i.count++;
		else if (*p == 'E')
			data->e.count++;
		else if (*p == ' ')
			manage_error(1, "Invalid Map: map is not rectangular");
		else
			manage_error(1, "Invalid Map: map contains invalid characters");
		p++;
	}
}

void	perform_map_checks(t_data *data)
{
	int	check;

	check = 0;
	check = check_boundaries(data->m);
	if (check == 1)
		manage_error(1, "Invalid Map: map is not surrounded by walls");
}
